...
impl Rect {
    ...
    fn choose<R: Rng>(bounds: Size, min_size: Size, max_size: Size, rng: &mut R) -> Self {
        let width = rng.random_range(min_size.width()..max_size.width());
        let height = rng.random_range(min_size.height()..max_size.height());
        let size = Size::new(width, height);
        let top_left_bounds = bounds - size;
        let left = rng.random_range(0..top_left_bounds.width());
        let top = rng.random_range(0..top_left_bounds.height());
...
fn l_shaped_corridor<R: Rng>(
    ...
    let axis = if rng.random() { Axis::X } else { Axis::Y };
...
impl RoomsAndCorridorsLevel {
    ...
    pub fn generate<R: Rng>(size: Size, rng: &mut R) -> Self {
        const NUM_ROOM_ATTEMPTS: usize = 1000;
        const MIN_ROOM_SIZE: Size = Size::new_u16(5, 5);
        const MAX_ROOM_SIZE: Size = Size::new_u16(20, 20);
